...
const REQUIRED_TABLES = ['api_keys', 'audit_events', 'checkpoints', 'schema_version'] as const;
...
export function inspectRecoveryCandidate(candidatePath: string): RecoveryInspection {
  ...
  try {
    ...
    const acceptableForRestore =
      integrityOk && requiredTablesPresent && historyRecovered && chain?.valid === true;
  ...
  return {
    ...inspection,
    evidence_files_unchanged: sameEvidence(before, evidenceFiles(candidate)),
  };
}
