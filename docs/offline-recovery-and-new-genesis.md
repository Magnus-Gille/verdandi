...
rsync target `/home/magnus/repos/verdandi` again.
...
5. With the service still inactive, move the validated trio into the canonical
   directory and set mode `0600`. Install the reviewed service unit. Do not
   enable the checkpoint timer yet.
...
`/home/magnus/.local/share/verdandi/genesis.json` first, containing:
...
5. Emit one explicit `system.audit_genesis`/incident event when that taxonomy
   is implemented; until then retain `genesis.json` beside the database.
...
- The service unit asserts that the directory already exists. A deployment
  cannot silently create a new empty generation.
